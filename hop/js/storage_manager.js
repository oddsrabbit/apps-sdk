...
// deliberately, since Hop has the same two boards and the same
...
(function () {
  ...
  function readKey(bridge, key) {
    return bridge.get(key).catch(function (err) {
      console.warn("hop: storage read failed for " + key, err);
      return null;
    });
  }
  ...
  StorageManager.prototype.hydrate = function () {
    ...
    return Promise.all([
      ...
      console.warn("hop: storage hydrate failed", err);
    });
  };
  ...
  window.HopStorageManager = StorageManager;
