...
// platform has already accepted. Same shape as match3's storage_manager —
// deliberately, since Hex Rush has the same two boards and the same
// submit-and-confirm retry loop. A real-time game's in-progress board has no
// meaningful "resume from move N" once the player has closed the page, so the
// stacks themselves are never saved.
...
(function () {
  ...
  // Sound preference. Separate from the score keys so a failed read of it can
  // never take a score down with it.
   ...
   * UTC+13 would start submitting to next month's board while UTC-11 players are
   * still filling this one, and the two would never see the same table.
   */
  ...
  // "YYYY-MM:1234" -> { period: "2026-09", score: 1234 }, or null if it doesn't
  ...
  // One key, read so that its failure can't take the others down with it.
  //
  ...
  // next run against it, so the first score of the session would read as a new
  // best and OVERWRITE the real one. A failed read of a submission marker costs
  // at most one server-deduped resubmit; a failed read of the best score,
  // propagated, costs the player the score itself.
  function readKey(bridge, key) {
    return bridge.get(key).catch(function (err) {
      console.warn("hex: storage read failed for " + key, err);
      return null;
    });
  }
  ...
  StorageManager.prototype.hydrate = function () {
    ...
    return Promise.all([
      ...
      readKey(self._bridge, MUTED_KEY),
      ...
      // so a malformed stored value can't stop the game booting.
      console.warn("hex: storage hydrate failed", err);
    });
  };
  ...
  StorageManager.prototype.isMuted = function () {
    return !!this._muted;
  };
  ...
  StorageManager.prototype.setMuted = function (muted) {
    this._muted = !!muted;
    if (this._bridge) {
      this._bridge.set(MUTED_KEY, muted ? "1" : "0").catch(function () {});
    }
  };

  window.HexStorageManager = StorageManager;
