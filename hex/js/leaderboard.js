// Hex Rush leaderboard modal — two global, public boards backed by the
// platform scores API:
...
// here is the Hex-Rush-specific part: which rounds, how a value reads, and the
...
//
// ES5-style vanilla JS to match the rest of hex (copied static, not bundled).
...
(function () {
  ...
  // Every tap on this game's board is a rotation, and an idle board treats
  // one as "start" (see _rotate in game.js). stopPropagation therefore isn't
  // cosmetic here: without it, opening the board would also start a run behind
  // the modal, or spin the hexagon during one.
  ...
  function openLeaderboard() {
    // Blocks keep falling behind a modal, so pause first — application.js
    // installs this hook once the game exists and it's a no-op outside a live
    // run. Absent until then (and if bootstrap failed), hence the typeof guard.
    if (typeof window.HexPauseForModal === "function") {
      try { window.HexPauseForModal(); } catch (_) {}
    }
