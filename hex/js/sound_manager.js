...
// shipping (and licensing) audio for four blips.
//
...
(function () {
  ...
  SoundManager.prototype.playRotate = function () {
    this._blip({ type: "triangle", from: 340, to: 300, dur: 0.05, gain: 0.045 });
  };

  SoundManager.prototype.playLand = function () {
    this._blip({ type: "square", from: 200, to: 150, dur: 0.07, gain: 0.05 });
  };

  // Pitch rises with the combo multiplier so a chain is audible as a climb
  // rather than as the same note repeated. Capped so a long chain doesn't end
  // up somewhere piercing.
  SoundManager.prototype.playClear = function (multiplier) {
    var step = Math.min(multiplier || 1, 8) - 1;
    var base = 520 * Math.pow(1.12, step);
    this._blip({ type: "square", from: base, to: base * 1.6, dur: 0.13, gain: 0.07 });
  };

  SoundManager.prototype.playGameOver = function () {
    this._blip({ type: "sawtooth", from: 300, to: 70, dur: 0.5, gain: 0.08 });
  };

  window.HexSoundManager = SoundManager;
})();
