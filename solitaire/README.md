...
Every UTC day, all players see the same shuffle. The day id is the integer number of UTC days since `EPOCH_MS` in `js/deck.js` (2026-09-09, moved at the re-release so the counter restarts at #1); a seed derived from it is run through a mulberry32 PRNG → Fisher-Yates shuffle. The daily leaderboard is keyed off the **day id**, so every player on a given day lands in the same round.

**The dealt seed is winnability-filtered** (`js/solver.js`). Because everyone shares one daily shuffle, an unwinnable board would break every player's streak at once and leave nobody able to tell a misplay from an impossible deal — so each day id maps to a deterministic *sequence* of candidate seeds (`Deck.dailySeedAt(id, k)`), and the deal uses the first one the solver can actually prove winnable. The sequence is a hash of `(id, k)` across the full 32-bit seed space rather than `id + k`, so one day's re-rolls never wander into the next day's: under `id + k`, any day that missed on its first attempt dealt the *next* day's opening seed, and since roughly half of all seeds fail to prove out, that repeated yesterday's board about half the time. The search is pure and deterministic, so every device resolves the same seed for a given day. It is sound but not complete: a deal it approves is definitely winnable, while a hard-but-winnable deal may be skipped in favour of the next seed — which biases the daily toward the more tractable end of winnable.
...
A **Random deal** button is available too — random deals skip the solver entirely (an unwinnable one costs nothing: no streak, just reroll) and do not contribute to the streak or to any leaderboard. A random deal is seeded from the clock, so nobody else has ever seen that shuffle and there is no field to place a time in; `finalizeWin` submits nothing for one and the win overlay says so (`Random deals aren't ranked`) rather than leaving a missing board looking like a failed fetch.

## Leaderboard

Two entry points, both `hidden` in the markup until `whenReady()` confirms the host serves `scores.top` or `scores.friends`: the trophy in the HUD, which is the only one reachable mid-deal, and a **Leaderboard** button on the chooser and won overlays, which is the only one reachable while `.game-message` is painted over the HUD.

Both open one modal: the boards for a single daily deal, with prev/next either side of the title scrubbing back up to seven days. A daily board is complete precisely when nobody is looking at it — the field fills over the day and the player who solved at 9am has closed the app before it settles — so past deals are most of the point. Today is included (unlike rabbit-words' past-round modal, which stops at yesterday to avoid spoiling a puzzle in progress; a solve time spoils nothing here).

`buildDailyTabs(id)` in `js/application.js` builds the tabs for an **arbitrary** deal id, and both the modal and the inline panel on the won overlay call it — so "which rounds, and what a score reads back as" is answered once and reaching a past deal is purely a different id. The season tab's period comes from the deal, not from the clock: in the first week of a month the picker reaches deals from the previous one.

**Opening the modal pauses the deal clock** (`game.pauseClock()`, released on close). Not a nicety — solve time *is* the ranking metric here, so reading the board would otherwise cost the player the number the board ranks them by.
...
- `bridge:scores` — daily deals submit to a per-deal leaderboard (`roundKey = daily2-{dayId}`; the `daily2-` prefix belongs to the re-released numbering — see `EPOCH_MS` in `js/deck.js` — and keeps the restarted counter off the `daily-` boards played before it). The score inverts solve time (faster = higher; `metadata = { timeMs, moves }`). Boards come from `scores.friends`, `scores.top` (+ `scores.rank` for the viewer's pinned placement) and `scores.season`, each tab added only where the host declares the verb. Anonymous players get a `requestSignIn` CTA; signed-in players with no friend scores yet get an invite CTA. Random deals have no shared round and skip scores entirely — see **Leaderboard** above.
