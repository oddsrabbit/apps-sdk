...
- `bridge:scores` — daily deals submit to a per-deal leaderboard (`roundKey = daily2-{seed}`; the `daily2-` prefix belongs to the re-released numbering — see `EPOCH_MS` in `js/deck.js` — and keeps the restarted counter off the `daily-` boards played before it). The score inverts solve time (faster = higher; `metadata = { timeMs, moves }`), and the won overlay renders a friends panel from `scores.friends`. Anonymous players get a `requestSignIn` CTA; signed-in players with no friend scores yet get an invite CTA. Random deals have no shared round and skip scores entirely.
