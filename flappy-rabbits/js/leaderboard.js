// Hop leaderboard modal — two global, public boards backed by the
// platform scores API:
...
// here is the Hop-specific part: which rounds, how a value reads, and the
...
//
// ES5-style vanilla JS to match the rest of hop (copied static, not bundled).
...
(function () {
  ...
  // Every tap on this game's board is a hop, and an idle board treats one as
  // "start" (see HopGame.tap). stopPropagation therefore isn't cosmetic here:
  // without it, opening the leaderboard would also start a run behind the
  // modal, or hop the rabbit during one.
  ...
  function openLeaderboard() {
    // The world keeps scrolling behind a modal, so pause first —
    // application.js installs this hook once the game exists and it's a no-op
    // outside a live run. Absent until then (and if bootstrap failed), hence
    // the typeof guard.
    if (typeof window.HopPauseForModal === "function") {
      try { window.HopPauseForModal(); } catch (_) {}
    }
